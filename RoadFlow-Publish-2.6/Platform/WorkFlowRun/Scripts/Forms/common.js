    ...
    initData: function (jsonData, dbtable, jsonStatus, displayModel)
    {
        ...
        $("[issubflowtable='1']").each(function ()
        {
            ...
            else
            {
                ...
                var ajaxUrl = "/Platform/WorkFlowRun/Scripts/Forms/GetSubTableData.aspx?secondtable=" + secondtable + "&primarytablefiled=" + primarytablefiled +
                ...
                $.ajax({
                    url: ajaxUrl, cache: false, async: false, type: "POST", data: { subtableformat: '[' + subtableFormats.join(',') + ']'}, dataType: "JSON", success: function (json)
                    {
        ...
        switch (type)
        {
            ...
            case "subflow_files":
                ...
                else if (filedshow == 1)
                {
                    ...
                    {
                        $.ajax({
                            url: "/Platform/Files/GetShowString.ashx", data: { "files": initValue }, type: "post", success: function (links)
                            {
                                $control.next().remove();
                                $control.prev().remove();
                                $control.after(links);
